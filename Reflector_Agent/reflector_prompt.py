...
评估流程：
1. 首先，阅读情景记忆步骤中的内容，了解前端返回的错误信息提示、出错时执行的动作、出错前后的界面xml结构内容。重点关注前后两个界面xml的内容差异，结合其他信息，分析出现问题的具体原因
2. 然后，综合我提供的信息，判断如果要处理出现的问题，当前界面是否需要回退
3. 最后，若当前界面不需要回退，判断问题类型是“选择了错误的区域”还是“执行了错误的动作”，并为下一步的区域选择或动作规划提出具体的建议。
4. 注意：使用提供的Memory Steps来理解每个步骤的上下文，它们按时间顺序排列
...
{{
    "need_back": false,
    "problem_type": "area",
    "advice": "使用'你'的形式提供直接建议，专注于当前情况 - 重试时你需要做什么",
    "summary": "发生了什么的简要总结"
}}
