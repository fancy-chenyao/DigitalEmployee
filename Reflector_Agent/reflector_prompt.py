...
2. 了解用户整体的指令、选择子任务前可用的子任务列表、当前正在执行的子任务、客户端返回的错误类型、执行返回的错误信息、客户端执行的子任务动作和执行子任务前后的界面信息。重点关注子任务执行前后前后两个界面xml的内容差异。
3. 综合提供的提示词信息，分析出现问题的具体原因，判断如果要处理出现的问题，当前界面是否需要回退
...
3. “执行子任务前后的界面信息”为当前子任务执行前后的界面信息，可以使用来理解每个步骤的上下文，它们按时间顺序排列
