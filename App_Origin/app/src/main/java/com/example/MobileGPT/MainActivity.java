...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean checkAccessibilityPermissions() {
        ...
        final String service = getPackageName() + "/" + "com.example.MobileGPT.MobileGPTAccessibilityService";
    ...
    public void setInstruction(String instruction) {
        Intent intent = new Intent(MobileGPTGlobal.STRING_ACTION);
        intent.putExtra(MobileGPTGlobal.INSTRUCTION_EXTRA, instruction);
        sendBroadcast(intent);
    }

    // 单应用模式：移除硬编码的应用选择按钮
    // 所有指令都将在当前应用内执行，不再需要指定特定应用
}